      ...
      >
        ...
        <div className="hidden items-center gap-8 lg:flex">
          <nav className="flex items-center gap-8 text-[12px] uppercase tracking-[0.24em] text-white/80">
            ...
            <a href="/#contact-info" className="transition hover:text-white">
              {navContact}
            </a>
          </nav>
          ...
          <button
            type="button"
            onClick={goToSampleRequest}
            className="relative flex items-center text-white/80 transition hover:text-white"
            aria-label="Sample Request"
          >
            <Package size={18} strokeWidth={1.6} />

            {sampleCount > 0 && (
              <span className="absolute -right-2.5 -top-2.5 flex h-5 min-w-5 items-center justify-center rounded-full bg-white px-1 text-[10px] font-medium leading-none text-[#2d241c] shadow">
                {sampleCount}
              </span>
            )}
          </button>
        ...
        <div className="fixed left-0 right-0 top-[64px] z-40 bg-black/80 px-8 py-8 text-white backdrop-blur-md lg:hidden">
          ...
          <div className="grid gap-6 text-sm uppercase tracking-[0.22em] text-white/80">
            ...
            <button
              ...
              className="relative flex w-fit items-center gap-3 text-sm uppercase tracking-[0.22em] text-white/80"
            >
              <Package size={18} strokeWidth={1.6} />
              <span>Sample Request</span>

              {sampleCount > 0 && (
                <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-white px-1.5 text-[10px] font-medium leading-none text-[#2d241c]">
                  {sampleCount}
                </span>
              )}
            </button>
